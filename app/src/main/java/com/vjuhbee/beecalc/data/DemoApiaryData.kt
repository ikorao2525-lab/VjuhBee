...
object DemoApiaryData {
    suspend fun seed(hiveRepository: HiveRepository, calendarRepository: CalendarRepository) {
        val now = System.currentTimeMillis()
        val today = LocalDate.now()
        val hives = listOf(
            Hive(0, "Улей №1 — Сильная семья", "Сильная семья, основная линия", UUID.randomUUID().toString(), now),
            Hive(0, "Улей №2 — Молодая матка", "Молодая матка, наблюдать засев", UUID.randomUUID().toString(), now),
            Hive(0, "Улей №3 — Отводок", "Отводок этого сезона", UUID.randomUUID().toString(), now),
            Hive(0, "Улей №4 — На наблюдении", "Требует дополнительного контроля", UUID.randomUUID().toString(), now)
        )
        ...
        val tasks = listOf(
            CalendarTask(0, today.year, today.monthValue, title = "Проверить кормовые запасы", shortDescription = "Осмотреть семьи и оценить остаток корма", category = TaskCategory.INSPECTION, importance = Importance.HIGH, dueDateMillis = today.atStartOfMillis(), linkedHiveUuids = savedHives.take(2).map { it.uuid }, uuid = UUID.randomUUID().toString(), updatedAt = now),
            CalendarTask(0, today.year, today.monthValue, title = "Подготовить рамки", shortDescription = "Подготовить рамки к расширению гнезда", category = TaskCategory.MAINTENANCE, dueDateMillis = today.plusDays(5).atStartOfMillis(), linkedHiveUuids = savedHives.map { it.uuid }, uuid = UUID.randomUUID().toString(), updatedAt = now),
            CalendarTask(0, today.year, today.monthValue, title = "Провести контрольный осмотр", shortDescription = "Проверить матку и расплод", category = TaskCategory.INSPECTION, dueDateMillis = today.minusDays(5).atStartOfMillis(), linkedHiveUuids = listOf(savedHives[1].uuid), uuid = UUID.randomUUID().toString(), updatedAt = now),
            CalendarTask(0, today.year, today.monthValue, title = "Обработка после сезона", shortDescription = "Выполнить обработку по инструкции препарата", category = TaskCategory.TREATMENT, importance = Importance.HIGH, dueDateMillis = today.minusDays(20).atStartOfMillis(), linkedHiveUuids = listOf(savedHives[3].uuid), uuid = UUID.randomUUID().toString(), updatedAt = now),
            CalendarTask(0, today.year, today.monthValue, title = "Откачка мёда", shortDescription = "Демонстрационная выполненная работа", category = TaskCategory.HARVEST, isDone = true, dueDateMillis = today.minusDays(2).atStartOfMillis(), harvestItems = listOf(HarvestItem(HarvestProduct.HONEY, 12.5, HarvestUnit.KG), HarvestItem(HarvestProduct.POLLEN, 1.8)), linkedHiveUuids = listOf(savedHives[0].uuid), uuid = UUID.randomUUID().toString(), updatedAt = now),
            CalendarTask(0, today.year, today.monthValue, title = "Работа без точной даты", shortDescription = "Пример старой задачи, привязанной только к месяцу", category = TaskCategory.OTHER, linkedHiveUuids = listOf(savedHives[2].uuid), uuid = UUID.randomUUID().toString(), updatedAt = now)
        )
